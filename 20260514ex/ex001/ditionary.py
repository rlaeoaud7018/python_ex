# 딕셔너리(dictionary)
'''
딕셔너리(dictionary)는 리스트와 함께 파이썬 프로그램에서 많이 사용하는 
컨테이너 자료형입니다. 
리스트가 인덱스를 이용하여 아이템을 참조한다면, 딕셔너리는 인덱스 대신 
키(key)를 이용하여 아이템을 참조(관리)합니다. 
'''
...
'''
#         키    밸류   키    백류   키    밸류   키    밸류    키    밸류
ages = {'박찬호': 48,'박지성': 40,'박세리': 50,'이승열': 43, '박찌성': 50}
print(f'ages: {ages}')
...
# 2
print(f'scores: {scores['Java']}')
print(f'scores: {scores['시스템']}')
